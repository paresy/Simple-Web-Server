...
namespace SimpleWeb {
  ...
  public:
    ...
    public:
      ...
      /// Deprecated, please use local_endpoint().port() instead.
      DEPRECATED std::string local_endpoint_address() const noexcept {
        try {
          if(auto connection = this->connection.lock())
            return connection->socket->lowest_layer().local_endpoint().address().to_string();
        }
        catch(...) {
        }
        return std::string();
      ...
      /// Deprecated, please use local_endpoint().port() instead.
      DEPRECATED unsigned short local_endpoint_port() const noexcept {
        try {
          if(auto connection = this->connection.lock())
            return connection->socket->lowest_layer().local_endpoint().port();
        }
        catch(...) {
        }
        return 0;
